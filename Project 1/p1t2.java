...
public class p1t2
{
	...
	*/
	public void search_movie_by_star() throws Exception
	{
		...
		{
			star_ids = db.select("SELECT id,first_name,last_name FROM stars WHERE id = " + tokes[0]);
		}
		else //search by name
		{
			if(tokes.length == 1)
			{
				...
				star_ids = db.select("SELECT id,first_name,last_name FROM stars WHERE first_name = \"" + tokes[0] + "\" OR last_name = \"" + tokes[0] + "\"");
			}
			else
			{
